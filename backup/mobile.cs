...
{    
    ...
    public class V2_DepositController : ApiController
    {
        ...
        private static readonly string PromoIconPath = "images/promotion/";
            ...
            if (SSoTokenHelper.Instance.ValidateSsoToken(param.Body.SSoToken, out userInfo, response))
            {
                ...
                try
                {
                    ...
                    if (pendingDepositList != null && pendingDepositList.Count > 0)
                    {
                        ...
                        var iconPO = displayLanguageSetting.Select(m => m.Icon).FirstOrDefault();
                        var iconNamePO = string.IsNullOrEmpty(displayLanguageSetting.Select(m => m.IconName).FirstOrDefault()) ? "default-paymentgateway.jpg" : displayLanguageSetting.Select(m => m.IconName).FirstOrDefault();
                        var iconPathPO = getPO_IconPath(iconPO, iconNamePO);
        ...
        [SwitchCultureActionFilter]
        public ResponseResult<V2_DepositInfoResponseModel> GetDepositInfo(RequestParam<DepositInfoRequestModel> param)
        {
            string scheme = HttpContext.Current.Request.UrlReferrer == null ? HttpContext.Current.Request.Url.Scheme : HttpContext.Current.Request.UrlReferrer.Scheme;

            PaymentLogger.InfoFormat("[GetDepositInfo] - SSOToken: [{0}], Channel: [{1}]", param.Body.SSoToken, param.Body.ChannelId);
            ResponseResult<V2_DepositInfoResponseModel> response = new ResponseResult<V2_DepositInfoResponseModel>();
            V2_DepositInfoResponseModel responseBody = new V2_DepositInfoResponseModel();
            ResponseCode responseCode = ResponseStatusCode.Fail;
            SessionInfo userInfo = new SessionInfo();
            ...
            if (SSoTokenHelper.Instance.ValidateSsoToken(param.Body.SSoToken, out userInfo, response))
            {
                try
                {
                    ...
                    PaymentOptionSettingEntity setting = lastUsedRawInfo == null ? null : PaymentSettingCacheManager.GetPaymentOptionSettingDetails((int)lastUsedRawInfo.paymentOptionID);
                    var displayLanguageSetting = lastUsedRawInfo == null ? null : setting.DisplayLanguageSetting.Where(m => m.LanguageCode == param.Body.LanguageCode);
                    string paymentOptionName = lastUsedRawInfo == null ? string.Empty : displayLanguageSetting.Select(m => m.DisplayName).FirstOrDefault() ?? lastUsedRawInfo.paymentOptionID.ToString();
                    var iconPO = lastUsedRawInfo == null ? null : displayLanguageSetting.Select(m => m.Icon).FirstOrDefault();
                    var iconNamePO = lastUsedRawInfo == null ? null : string.IsNullOrEmpty(displayLanguageSetting.Select(m => m.IconName).FirstOrDefault()) ? "default-paymentgateway.jpg" : displayLanguageSetting.Select(m => m.IconName).FirstOrDefault();
                    var iconPathPO = iconPO == null ? string.Empty : getPO_IconPath(iconPO, iconNamePO);
                    var bank = lastUsedRawInfo == null ? null : BankingManager.GetBankById(lastUsedRawInfo.bankId);
                    

                    // Get Last Deposit Transaction
                    responseBody.LastTransaction = lastUsedRawInfo == null ? null : new LastDepositTransactionContent()
                    {
                        TransactionNumber = lastUsedRawInfo.transactionNo,
                        Currency = lastUsedRawInfo.currencyCode,
                        TransactionDate = lastUsedRawInfo.dateCreated.AddHours(ServerTimeZone * -1).AddMinutes(DateUtil.ConvertTimeZoneToMinutes(param.Body.TimeZone)),
                        Amount = lastUsedRawInfo.grossAmount + lastUsedRawInfo.adjustmentAmount + lastUsedRawInfo.reimbursementAmount,
                        BankName = userInfo.CurrencyCode.Equals("USDT") ? bank.BankCode : (bank.BankName ?? string.Empty),
                        TransactionStatusID = (int)lastUsedRawInfo.paymentDepositStatus,
                        TransactionStatusName = lastUsedRawInfo.paymentDepositStatus.ToString(),
                        IconImagepath = iconPathPO,
                        PaymentType = (int)lastUsedRawInfo.paymentType,
                        DepositMethod = userInfo.CurrencyCode.Equals("USDT") ? string.Format("{0}-{1}", paymentOptionName, bank.BankCode) : paymentOptionName,
                        IsAbleToCancel = IsAbleToCancelDepositTransaction(lastUsedRawInfo.paymentDepositStatus),
                    };

                    // Get receipts of last deposit transaction
                    var detailVo = lastUsedRawInfo == null ? null : PaymentDepositManager.Instance.GetPaymentTransactionDetail(lastUsedRawInfo.transactionNo);

                    if (detailVo != null && detailVo.LstPaymentGatewayDetailVO != null && detailVo.LstPaymentGatewayDetailVO.Count > 0)
                    {
                        if (responseBody.LastTransaction != null)
                        {
                            var url = HttpContext.Current.Request.Url;
                            string receiptPath = AppConfigManager.SystemSetting.UploadReceiptPath;

                            responseBody.LastTransaction.Receipts = new List<ReceiptModel>();
                            var receiptList = detailVo.LstPaymentGatewayDetailVO.Where(x =>
                            x.PaymentTransDetailMapId.Equals(PaymentTransactionDetailConstant.DP_RECEIPT_IMAGE_NAME_1) ||
                            x.PaymentTransDetailMapId.Equals(PaymentTransactionDetailConstant.DP_RECEIPT_IMAGE_NAME_2) ||
                            x.PaymentTransDetailMapId.Equals(PaymentTransactionDetailConstant.DP_RECEIPT_IMAGE_NAME_3));
                            ...
                            foreach (var receipt in receiptList)
                            {
                                if (!string.IsNullOrEmpty(receipt.value) && receipt.value.Split('|').Length >= 2)
                                {
                                    ...
                                    responseBody.LastTransaction.Receipts.Add(new ReceiptModel()
                                    {
                                        FileOriginalName = receipt.value.Split('|')[0],
                                        FileName = receipt.value.Split('|')[1],
                                        Base64Content = getBase64ContentForAttachment(receiptPath, fileName),
                    ...
                    foreach (var paymentOption in paymentOptionList)
                    {
                        ...
                        if (paymentTypeWithBankList.Count > 0 && isPaymentOptionActivateByChannel)
                        {
                            ...
                            setting = PaymentSettingCacheManager.GetPaymentOptionSettingDetails((int)paymentOption.PaymentOptionID);
                            displayLanguageSetting = setting.DisplayLanguageSetting.Where(m => m.LanguageCode == param.Body.LanguageCode);
                            ...
                            var limitSetting = PaymentManagerBLL.GetMemberPaymentLimitByPaymentOptionID(userInfo.UserID, paymentOption.PaymentOptionID, userInfo.CurrencyCode, transactionType);

                            item.LeftDailyCount = limitSetting.LimitCount.HasValue ? ((limitSetting.LimitCount - limitSetting.CurrentCount) > 0 ? (limitSetting.LimitCount - limitSetting.CurrentCount) : 0) : 99;
                            item.TotalDailyCount = limitSetting.LimitCount.HasValue ? ((limitSetting.LimitCount - limitSetting.CurrentCount) > 0 ? (limitSetting.LimitCount - limitSetting.CurrentCount) : 0) : 99;
                            item.IsAvailable = item.LeftDailyCount == 0 ? false : true;
                    ...
                    var promotionAvailable = Manager.ModulePromotion.DepositPromotionRecommendation.DepositPromotionRecommendation.Instance.GetDepositPromotionRecommendationList(memberEntity.currencyCode, lastUsedRawInfo == null ? false : true);
                   ...
                   // bool POIsMaintain = paymentOptionMaintainInfo.recurringList.Where(x => x.Status).Count() > 0 && (DateTime.UtcNow.AddHours(ServerTimeZone) >= paymentOptionMaintainInfo.ActiveStartDate && DateTime.UtcNow.AddHours(ServerTimeZone) <= paymentOptionMaintainInfo.ActiveEndDate);
                    ...
                    foreach (var receipt in paymentOptionMaintainInfo.recurringList)
                    {
                        ...
                        if (receipt.Status == true)
                        {
                            ...
                            {
                                DateTime today = DateTime.UtcNow.AddHours(ServerTimeZone);
                    ...
                    else if (responseBody.IsReachedDailyLimit || responseBody.IsReachedOverallDailyLimit)
                    {
                        responseBody.ErrorMessage = new MessageMaintenance()
                        {
                    ...
                    else if (responseBody.IsReachedPOAmountLimit || responseBody.IsReachedOverallAmountLimit)
                    {
                        responseBody.ErrorMessage = new MessageMaintenance()
                        {
                    ...
                    else if (paymentTypeWithBankList.Count > 0 || !isSupportRoutingRules)
                    {
                        ...
                        // Get FTD or ReDeposit
                        var lastUsedRawInfo = PaymentDepositManager.Instance.GetLastPaymentTransaction(userInfo.AccountID);
                        var promotionAvailable = Manager.ModulePromotion.DepositPromotionRecommendation.DepositPromotionRecommendation.Instance.GetDepositPromotionRecommendationList(userInfo.CurrencyCode, lastUsedRawInfo == null ? false : true);
        ...
        public ResponseResult<ResponseBase> CancelPending(RequestParam<CancelDepositTransactionRequestModel> param)
        {
            ...
            if (SSoTokenHelper.Instance.ValidateSsoToken(param.Body.SSoToken, out userInfo, response))
            {
                ...
                if (deposit.paymentDepositStatus.Equals(PaymentEnum.PaymentDepositStatus.Pending))
                {
                    ...
                    {
                        responseCode = ResponseStatusCode.DPDataNotMatched;
                    }
                }
                    ...
                    foreach (var bank in supportedBankList)
                    {
                        if (item.bankID == bank.BankID)
                        {
                            var bankAccount = new MemberBankAccountEntityViewModel()
                            {
                                ...
                                BankAccountNumberUnmaskedDisplay = item.bankAccountNo,
        ...
        private string getPromoIconPath(CurrencyEnum.CurrencyCode? code)
        {
            string path = Path.Combine(PromoIconPath, code.Equals(CurrencyEnum.CurrencyCode.RMB) ? "icon_FTD_RMB.jpg" : code.Equals(CurrencyEnum.CurrencyCode.USDT) ? "icon_FTD_USDT.jpg" : "default-promo.jpg");
            return path;
        }
